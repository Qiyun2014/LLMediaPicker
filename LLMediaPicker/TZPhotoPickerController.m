...
@property (strong, nonatomic) TZAssetModel  *editorModel;
@end
...
- (void)presentToVideoEditorControllerWithAsset:(PHAsset *)asset{
    
    if (@available(iOS 8.0, *)) {
        PHContentEditingInputRequestOptions *requestOption = [[PHContentEditingInputRequestOptions alloc] init];
        requestOption.networkAccessAllowed = YES;
        requestOption.progressHandler = ^(double progress, BOOL *stop){
            NSLog(@"progress = %f",progress);
        };
        [asset requestContentEditingInputWithOptions:requestOption
                                   completionHandler:^(PHContentEditingInput *contentEditingInput, NSDictionary *info) {
                                       dispatch_async(dispatch_get_main_queue(), ^{
                                           
                                           NSURL *fileUrl = [contentEditingInput valueForKey:@"videoURL"];
                                           if (fileUrl.path) {
...
- (void)dismissEditorControllerWithCompletion:(void (^) (void))completion{
    dispatch_async(dispatch_get_main_queue(), ^{
...
{
    [self dismissEditorControllerWithCompletion:^{
        TZVideoPlayerController *videoPlayerVc = [[TZVideoPlayerController alloc] init];
        videoPlayerVc.model = nil;
        [self.navigationController pushViewController:videoPlayerVc animated:YES];
    }];
}

- (void)videoEditorController:(UIVideoEditorController *)editor didFailWithError:(NSError *)error{
    [self dismissEditorControllerWithCompletion:nil];
    NSLog(@"编辑失败 ...");
}
...
- (void)addPHAsset:(PHAsset *)asset {
    ...
    if (tzImagePickerVc.maxImagesCount <= 1) {
        if (tzImagePickerVc.allowCrop && asset.mediaType == PHAssetMediaTypeImage) {
            ...
            if (assetModel.asset) {
                self.editorModel = assetModel;
                [self presentToVideoEditorControllerWithAsset:assetModel.asset];
            }
        }
        return;
    }
